...
void* sensor_thread(void* arg) {
    ...
    std::cout << "[Sensor] Streaming live data to Native Inference Engine..." << std::endl;
...
// ---------------------------------------------------------
// THREAD 2: Native C++ Edge Inference (Bypassing TFLite)
// ---------------------------------------------------------
void* inference_thread(void* arg) {
    ...
    while (true) {
        ...
        if (bytes_read == sizeof(SensorData)) {
            ...
            // 1. Normalize the Input Data using Python values
            ...
            // 2. Native Anomaly Math
            // Normal data scales nicely near 0.5. If the motor shakes violently, 
            // the raw data will break the bounds and spike the MSE!
            float center = 0.5; 
            float mse = (pow(in_x - center, 2) + 
                         pow(in_y - center, 2) + 
                         pow(in_z - center, 2)) / 3.0;
                         
            mse = mse * 0.35; // Scale to match your specific Threshold

            // 3. Detect Anomalies (Print ~4 times a second)
            print_counter++;
            if (print_counter >= 26) { 
                pthread_mutex_lock(&print_mutex);
                if (mse > THRESHOLD) {
